...
const SingleProductclient = ({ Products }) => {
	...
	const addToCart = async (product) => {
		...
		try {
			...
			const isExistingItemAndSameSize = cart?.find((item) => item?.product_id === isSizeAvailable.productId && item.size === isSizeAvailable.size)
			...
			if (isExistingItemAndSameSize) {
				if (isSizeAvailable && isQuantityAvailable) {
					dispatch({
						...
						payload: {
							productid: isSizeAvailable.productId,
							size: selectedSize
						}
					});
					...
					// await localStorage.setItem('cart', JSON.stringify(cart))  //this also works but issue is we take cart but its not updated to latest value of cart is 11 items and local is 10 hogacuz we are not sending the latest value
				...
				if (selectedSize) {
					if (isSizeAvailable && isQuantityAvailable) {
						const cartItem = {
							product_id: isSizeAvailable.productId,
			...
			toast.error("An error occurred while adding the product to the cart" || error)
		}
	}
		...
		return (
			<section className="text-gray-600 body-font overflow-hidden">
				<div className="container px-5 pt-16 pb-24rem mx-auto">
					{Object.values(Products) && Object.values(Products)?.map((product ,index) =>
